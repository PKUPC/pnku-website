...
from src import secret, adhoc, store
...
formatter = logging.Formatter(
    f'[%(asctime)s] [checker] [%(levelname)s] %(message)s',
...
class Checker:
    ...
    async def check_action(self, req: glitter.ActionReq, http_req: Request | None = None) -> dict[str, str] | None:
        ...
        listener: Callable[
            [Any, glitter.ActionReq, Request | None], Awaitable[dict[str, str] | None]] = self.action_listeners.get(
            type(req),
            default
        )
    ...
    async def on_team_submit_answer(
        ...
        if user.team.game_status.puzzle_visible_status(puzzle.model.key) != "unlock":
            store_user_log(
                http_req, "checker.submit_answer", "abnormal",
                f"试图给没有解锁的题目提交答案。", {"puzzle_key": req.puzzle_key, "content": req.content}
            )
        ...
        if user.team.preparing:
            store_user_log(
                http_req, "checker.submit_answer", "abnormal",
                f"试图在队伍未开始游戏时提交答案。", {"puzzle_key": req.puzzle_key, "content": req.content}
            )
            return {"status": "error", "title": "NOT_FOUND", "message": "题目不存在"}

        if (utils.clean_submission(req.content)
                in user.team.game_status.get_submission_set(req.puzzle_key)):
    ...
    async def on_team_buy_hint(
        ...
        if user.team.preparing:
            store_user_log(
                http_req, "checker.on_team_buy_hint", "abnormal",
                f"试图在队伍开始游戏前购买提示。", {"puzzle_key": req.puzzle_key, "hint_id": req.hint_id}
            )
            return {"status": "error", "title": "NOT_FOUND", "message": "题目不存在"}
        ...
        if hint_id not in self.game.hints.hint_by_id:
            store_user_log(
                http_req, "checker.on_team_buy_hint", "abnormal",
                f"试图购买不存在的提示。", {"puzzle_key": req.puzzle_key, "hint_id": req.hint_id}
            )
        ...
        if unlock_puzzle_ts + hint_cd > current_ts:
            store_user_log(
                http_req, "checker.on_team_buy_hint", "abnormal",
                f"试图购买还没到冷却时间的提示。", {"puzzle_key": req.puzzle_key, "hint_id": req.hint_id}
            )
    ...
    async def on_team_create_ticket(
            self,
            req: glitter.TeamCreateTicketReq,
            http_req: Request | None = None
    ) -> dict[str, str] | None:
        # 判断当前未完成的提示数量
        user = self.game.users.user_by_id[req.user_id]
        ...
        if req.ticket_type == "MANUAL_HINT":
            ...
            if user.team.game_status.unlock_puzzle_keys[puzzle_key] + adhoc.MANUAL_HINT_COOLDOWN > time.time():
                store_user_log(
                    http_req, "checker.on_team_create_ticket", "abnormal",
                    f"试图在还不能购买人工提示的时候购买人工提示。", {"puzzle_key": puzzle_key, "subject": req.subject}
                )
            ...
            if count > 0:
                return {"status": "error", "title": "REJECTED",
    ...
    async def on_set_ticket_status(
            self,
            req: glitter.SetTicketStatusReq,
            http_req: Request | None = None
    ) -> dict[str, str] | None:
        # 判断当前未完成的提示数量
        user = self.game.users.user_by_id[req.user_id]
        ...
        if not user.is_staff and not ticket.staff_replied and req.status == store.TicketStore.TicketStatus.CLOSED.name:
            store_user_log(
                http_req, "checker.on_set_ticket_status", "abnormal",
                f"试图在工作人员回复人工提示前关闭人工提示。", {"ticket_id": ticket.model.id}
            )
            return {"status": "error", "title": "REJECTED",
    ...
    async def on_send_message(
        ...
        if not user.is_staff and user.team.model.extra_info.ban_list.ban_message_until_ts > time.time():
            store_user_log(http_req, "api.message.send_message", "abnormal", f"被禁止发送站内信的队伍发送了站内信。",
