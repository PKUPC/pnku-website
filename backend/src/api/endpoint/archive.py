...
from sanic import Blueprint, HTTPResponse, response, Request

from src import secret, utils, adhoc
...
if secret.USE_ARCHIVE_API:
    ...
    async def game_info_json(_req: Request, worker: Worker) -> HTTPResponse:
        ...
        unlock_areas = [adhoc.areas.data.AREA_LIST[2],
    ...
    async def misc_json(_req: Request, worker: Worker) -> HTTPResponse:
        result_dict = {
            ...
            "schedule": [{
                ...
                'name': trigger.name,
                'status': (
                    'prs' if trigger.tick == worker.game_nocheck.cur_tick else
                    'ftr' if trigger.tick > worker.game_nocheck.cur_tick else 'pst'
                ),
    ...
    async def templates_json(_req: Request, worker: Worker) -> HTTPResponse:
        template_list = [
            "acknowledgements", "faq", "introduction", "tools", "endoftime", "ending", "prologue", "day3_premeta",
            "staff", "day1_intro", "day1_meta", "day2_intro", "day2_meta", "day3_intro", "day3_meta1", "day3_meta2",
            "day3_meta3",
        ]
    ...
    async def area_detail_json(_req: Request, worker: Worker) -> HTTPResponse:
        ...
        result_dict = {
            area_name: adhoc.get_area_info(area_name, user_1, worker)
