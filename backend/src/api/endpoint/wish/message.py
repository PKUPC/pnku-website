...
from src import utils, secret, adhoc
...
async def send_message(req: Request, body: SendMsgParam, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
    assert user.team is not None

    if secret.PLAYGROUND_MODE:
        store_user_log(req, "api.message.send_message", "abnormal", "在 playground 模式下发送站内信。",
...
async def read_message(req: Request, body: ReadMsgParam, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
    assert user.team is not None

    if user.is_staff:
        store_user_log(req, "api.message.read_message", "abnormal",
                       f"staff 试图调用这个 API。", {"team_id": body.team_id, "msg_id": body.msg_id})
        return {"status": "error", "title": "BAD_REQUEST", "message": "staff 不能调这个接口"}

    if user.model.team_id != body.team_id:
        store_user_log(req, "api.message.read_message", "abnormal",
                       f"用户提交的队伍 id 不是用户自己的队伍 id。", {"team_id": body.team_id, "msg_id": body.msg_id})
        return {"status": "error", "title": "BAD_REQUEST", "message": "队伍id出错"}

    rep = await worker.perform_action(glitter.TeamReadMsgReq(
        client=worker.process_name,
        team_id=body.team_id,
        msg_id=body.msg_id,
        direction=MessageStore.DIRECTION.TO_USER if user.is_staff else MessageStore.DIRECTION.TO_STAFF,
...
@wish_response
@wish_checker(["player_in_team", "intro_unlock"])
async def get_message(req: Request, body: GetMsgParam, worker: Worker, user: User | None) \
        -> dict[str, Any]:
    assert user is not None
    assert user.team is not None
    ...
    if body.start_id < 0:
        store_user_log(
            req, "api.message.get_message", "abnormal",
            "提供的 start_id 不合法。", {"team_id": body.team_id, "start_id": body.start_id}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "提供的消息id错误"}

    if not user.is_staff and user.model.team_id != body.team_id:
        store_user_log(
            req, "api.message.submit_answer", "abnormal",
            f"提供的队伍 id 和用户的队伍 id 不一致。", {"team_id": body.team_id, "start_id": body.start_id}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "队伍id出错"}
