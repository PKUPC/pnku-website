...
async def time_garden(req: Request, body: ChangeTimeParam, worker: Worker, user: User | None) -> dict[str, Any]:
    ...
    if not user.is_staff:
        if "day2_02" not in user.team.game_status.unlock_puzzle_keys:
            store_user_log(
                req, "api.special.day2_02", "abnormal", "试图在 day2_02 解锁前调用对应接口。",
    ...
    if body.hour < 0 or body.hour > 23 or body.minute < 0 or body.minute > 59:
        store_user_log(
            req, "api.special.day2_02", "abnormal", "给接口发送了非法的时间。",
            {"hour": body.hour, "minute": body.minute}
        )
        return {"status": "error", "title": "ABNORMAL", "message": f"{body.hour}:{body.minute} 是一个非法时间！"}

    store_user_log(
        req, "api.special.day2_02", "time_garden", "",
        {"hour": body.hour, "minute": body.minute}
    )

    rep = await worker.perform_action(glitter.PuzzleActionReq(
        ...
        content={
            ...
            "target_seconds": (body.hour * 60 + body.minute) * 60
        }
    ))
