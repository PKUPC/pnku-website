...
async def time_machine(req: Request, body: TimeMachineParam, worker: Worker, user: User | None) -> dict[str, Any]:
    ...
    if ms_diff < 3:
        return {'status': 'error', 'title': 'FREQUENCY_LIMIT', 'message': f'请求太快啦！请{3 - ms_diff:.1f} 秒后重试'}

    store_user_log(
        req, 'api.special.day2_05', 'time_machine', '', {'year': body.year, 'month': body.month, 'day': body.day}
    )
