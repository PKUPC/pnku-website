...
from src.state import User, Ticket, Submission
...
@wish_response
@wish_checker(["user_is_staff"])
async def get_team_detail(req: Request, body: GetTeamDetailParam, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
    assert user.is_staff
    ...
    return {
        ...
        } for idx, sub in enumerate(team.submissions)][::-1],
        "passed_puzzles": [
            {
                "title": p.model.title,
                "timestamp_s": int(s.store.created_at / 1000)
            }
            for p, s in team.passed_puzzles
        ],
        "ban_list": {
...
@wish_response
@wish_checker(["user_is_staff"])
async def v_me_50(req: Request, body: VMe50Param, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
    assert user.is_staff
    ...
    if body.team_id == 0:
        store_user_log(req, "api.staff.v_me_50", "abnormal", f"某个 staff 试图变动 staff 队伍的注意力。",
    ...
    rep = await worker.perform_action(glitter.VMe50Req(
    ...
    store_user_log(req, "api.staff.v_me_50", "staff_v_me_50", "", {
        "team_id": body.team_id, "ap_change": body.ap_change, "reason": body.reason
    })
...
async def v_me_100(req: Request, body: VMe100Param, worker: Worker, user: Optional[User]) -> dict[str, Any]:
    ...
    rep = await worker.perform_action(glitter.VMe100Req(
        client=worker.process_name,
        staff_id=user.model.id,
        team_id=body.team_id,
    ...
    store_user_log(req, "api.staff.v_me_100", "staff_v_me_100", "", {
...
async def get_submission_list(
    ...
    def _filter(x: Submission) -> bool:
        return ((body.team_id is None or x.team.model.id in body.team_id)
    ...
    if body.sort_field is not None and body.sort_order is not None:
        if body.sort_field == "timestamp_s":
            sorted_submissions = sorted(filtered_submissions, key=lambda x: x.store.created_at,
                                        reverse=(body.sort_order == "descend"))
    else:
    ...
    return {
        "data": {
            ...
            "list": [{
                ...
                enumerate(sorted_submissions[body.start_idx - 1:body.start_idx + body.count - 1])]
        }
    }
...
async def get_tickets(
        req: Request, body: GetTicketListParam, worker: Worker, user: Optional[User]
) -> dict[str, Any]:
    assert user is not None
    assert user.is_staff
    ...
    if body.sort_order is not None and body.sort_field is not None:
        if body.sort_field == "last_message_ts":
            sorted_tickets = sorted(filtered_tickets, key=lambda x: x.last_message_ts,
                                    reverse=(body.sort_order == "descend"))
    else:
    ...
    return {
        "status": "success", "title": "SUCCESS", "message": "完成！",
        "data": {
            "total_num": len(sorted_tickets),
            "list": list_result
        }
    }
...
async def update_extra_team_info(req: Request, body: UpdateExtraTeamInfoParam, worker: Worker, user: Optional[User]) \
    ...
    store_user_log(req, "api.staff.update_extra_team_info", "update_extra_team_info", "",
