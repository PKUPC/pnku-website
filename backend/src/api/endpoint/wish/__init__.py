...
from typing import Callable, Dict, Any, Union, Awaitable, Optional, Literal
            ...
            for check in inner_check_list:
                if check == "user_login":
                    ...
                    if not user.is_staff and not worker.game_nocheck.is_game_begin():
                        store_user_log(
                            req, "api.wish_checker.game_start", "abnormal",
                            "在游戏开始前调用了禁止的 API。", {"req_url": req.url})
                    ...
                    if not user.is_staff and not worker.game_nocheck.is_intro_unlock():
                        store_user_log(
                            req, "api.wish_checker.intro_unlock", "abnormal",
                            "在游戏开始前调用了禁止的 API。", {"req_url": req.url})
                        return {"status": "error", "title": "BAD_REQUEST", "message": "游戏未开始！"}
                    ...
                    if not user.is_staff:
                        store_user_log(req, "api.wish_checker.user_is_staff", "abnormal",
                                       "非工作人员访问了工作人员 API。", {"req_url": req.url})
                    ...
                    if not user.is_staff:
                        ...
                        if not user.team.gaming:
                            store_user_log(req, "api.wish_checker", "abnormal",
                                           "队伍没在游戏中。", {"req_url": req.url})
                            return {"status": "error", "title": "BAD_REQUEST", "message": "你不是工作人员！"}
...
from . import team, message, puzzle, staff, game, user, special, ticket, upload

bp = Blueprint.group(  # type: ignore[has-type]
    team.bp, message.bp, puzzle.bp, staff.bp, game.bp, user.bp, special.bp, ticket.bp,  # type: ignore[has-type]
