...
from src import secret, adhoc, utils
...
async def create_team(req: Request, body: CreateTeamParam, worker: Worker, user: Optional[User]) \
        -> Dict[str, Any]:
    ...
    if not utils.check_string(body.team_info):
        return {"status": "error", "title": "BAD_NAME", "message": "队伍简介包含非法 unicode 控制符，请不要做奇怪的尝试"}
    ...
    rep = await worker.perform_action(glitter.UserCreateTeamReq(
    ...
    store_user_log(req, "api.team.create_team", "create_team", "",
...
async def update_team(req: Request, body: UpdateTeamInfoParam, worker: Worker, user: Optional[User]) \
        -> Dict[str, Any]:
    assert user is not None
    ...
    if user.team.model.leader_id != user.model.id:
        store_user_log(req, "api.team.update_team", "abnormal", "非队长试图更改信息。",
                       {"team_name": body.team_name, "team_info": body.team_info, "team_secret": body.team_secret})
    ...
    if user.team.gaming and body.team_name != user.team.model.team_name:
        return {"status": "error", "title": "BAD_REQUEST",
    ...
    rep = await worker.perform_action(glitter.TeamUpdateInfoReq(
        client=worker.process_name,
        uid=user.model.id,
    ...
    store_user_log(req, "api.team.update_team", "update_team", "",
                   {"team_name": body.team_name, "team_info": body.team_info, "team_secret": body.team_secret})
...
@wish_response
@wish_checker(["player_in_team"])
async def update_extra_team_info(req: Request, body: UpdateExtraTeamInfoParam, worker: Worker, user: Optional[User]) \
        -> Dict[str, Any]:
    assert user is not None
    ...
    # 如果已经在队里，必须要是队长
    if user.team.model.leader_id != user.model.id:
        store_user_log(req, "api.team.update_extra_team_info", "abnormal", "非队长试图更改信息。",
    ...
    if body.type == "recruitment":
        ...
        if user.team.model.extra_info.ban_list.ban_recruiting_until_ts > time.time():
            store_user_log(req, "api.team.update_extra_team_info", "abnormal", f"被禁止使用招募的队伍试图修改招募信息。",
                           {"type": body.type, "data": body.data})  # type: ignore
            return {"status": "error", "title": "BAD_REQUEST", "message": "你的队伍被禁止发送站内信！"}

    rep = await worker.perform_action(glitter.TeamUpdateExtraTeamInfoReq(
        client=worker.process_name,
        # tid 这里应该保证存在
        uid=user.model.id,
        tid=user.team.model.id,
    ...
    store_user_log(req, "api.team.update_extra_team_info", "update_extra_team_info", "",
                   {"type": body.type, "data": body.data})  # type: ignore
...
@wish_response
@wish_checker(["user_login"])
async def join_team(req: Request, body: JoinTeamParam, worker: Worker, user: Optional[User]) \
        -> Dict[str, Any]:
    assert user is not None
    ...
    if body.team_id == 0:
        store_user_log(req, "api.team.join_team", "abnormal", "试图加入非法队伍！",
                       {"team_id": body.team_id, "team_secret": body.team_secret})
        return {"status": "error", "title": "BAD_REQUEST", "message": "试图加入非法队伍！"}

    rep = await worker.perform_action(glitter.UserJoinTeamReq(
        client=worker.process_name,
        tid=body.team_id,
        uid=user.model.id,
        team_secret=body.team_secret
    ...
    store_user_log(req, "api.team.join_team", "join_team", "",
                   {"team_id": body.team_id, "team_secret": body.team_secret})
...
@wish_response
@wish_checker(["player_in_team"])
async def leave_team(req: Request, worker: Worker, user: Optional[User]) \
        -> Dict[str, Any]:
    assert user is not None
    assert user.team is not None
    ...
    rep = await worker.perform_action(glitter.UserLeaveTeamReq(
        client=worker.process_name,
        tid=team_id,
        uid=user.model.id,
    ))
...
@wish_response
@wish_checker(["player_in_team"])
async def remove_user(req: Request, body: RemoveUserParam, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
    assert user.team is not None
    ...
    rep = await worker.perform_action(glitter.TeamRemoveUserReq(
        client=worker.process_name,
        from_id=user.model.id,
        tid=team_id,
        uid=body.user_id
    ))
...
@wish_response
@wish_checker(["player_in_team"])
async def change_leader(req: Request, body: ChangeLeaderParam, worker: Worker, user: Optional[User]) \
        -> Dict[str, Any]:
    assert user is not None
    assert user.team is not None
    if worker.game_nocheck.is_game_end():
    ...
    rep = await worker.perform_action(glitter.TeamChangeLeaderReq(
        client=worker.process_name,
        from_id=user.model.id,
        tid=user.model.team_id,
        uid=body.user_id
    ))
    ...
    store_user_log(req, "api.team.change_leader", "change_leader", "",
...
async def get_ap_change_history(req: Request, worker: Worker, user: Optional[User]) \
        -> Dict[str, Any]:
    assert user is not None
    if user.model.group == "staff":
    ...
    assert user.team is not None
    res = {
        "history": user.team.get_ap_change_list()
    }
...
async def get_submission_history(req: Request, worker: Worker, user: Optional[User]) -> dict[str, Any]:
    ...
    res = {
        "data": {
            ...
            } for idx, sub in enumerate(user.team.submissions)][::-1],
            "passed_submissions": [{
                'timestamp_s': int(sub.store.created_at / 1000),
