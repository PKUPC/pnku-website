...
from src import utils, adhoc, secret
from src.custom import store_user_log
from src.logic import Worker, glitter
from src.state import User
from src.store import TicketStore, ManualHintModel, TicketMessageStore
...
async def request_hint(
        req: Request, body: RequestHintParam, worker: Worker, user: Optional[User]
) -> dict[str, Any]:
    assert user is not None
    assert user.team is not None

    if secret.PLAYGROUND_MODE:
        store_user_log(req, "api.ticket.request_hint", "abnormal", "在 playground 模式下申请神谕。",
    ...
    if user.team.model.extra_info.ban_list.ban_manual_hint_until_ts > time.time():
        store_user_log(req, "api.ticket.request_hint", "abnormal", f"被禁止使用神谕的队伍试图申请神谕。",
    ...
    if puzzle_key is None:
        store_user_log(
            req, "api.ticket.request_hint", "abnormal", "unhash puzzle_key 失败。",
            {"body_puzzle_key": body.puzzle_key}
        )
    ...
    if team_id != user.team.model.id:
        store_user_log(
            req, "api.ticket.request_hint", "abnormal", "提交了其他队伍的 puzzle_key。",
    ...
    if puzzle_key not in user.team.game_status.unlock_puzzle_keys:
        store_user_log(
            req, "api.ticket.request_hint", "abnormal", "题目未解锁。",
            {"puzzle_key": puzzle_key}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}

    subject = f"『请求神谕』" + worker.game_nocheck.puzzles.puzzle_by_key[puzzle_key].model.title

    rep = await worker.perform_action(glitter.TeamCreateTicketReq(
...
async def get_manual_hints(
    ...
    if puzzle_key is None:
        store_user_log(
            req, "api.ticket.get_manual_hints", "abnormal", "unhash puzzle_key 失败。",
            {"body_puzzle_key": body.puzzle_key}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
    # 是其他队伍的 puzzle_key
    if team_id != user.team.model.id:
        store_user_log(
            req, "api.ticket.get_manual_hints", "abnormal", "提交了其他队伍的 puzzle_key。",
            {"body_puzzle_key": body.puzzle_key, "real_puzzle_key": puzzle_key, "another_team_id": team_id}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
    # 此时 puzzle_key 一定是存在的
    if puzzle_key not in user.team.game_status.unlock_puzzle_keys:
        store_user_log(
            req, "api.ticket.get_manual_hints", "abnormal", "题目未解锁。",
            {"puzzle_key": puzzle_key}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
...
async def get_ticket_detail(
    ...
    if ticket is None:
        store_user_log(
            req, "api.ticket.get_ticket_detail", "abnormal", "访问了不存在的工单。",
            {"ticket_id": body.ticket_id}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "不存在的数据！"}
    if not user.is_staff and user.team.model.id != ticket.team.model.id:
        store_user_log(
            req, "api.ticket.get_ticket_detail", "abnormal", "试图访问不属于自己队伍的工单。",
            {"ticket_id": body.ticket_id}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "不存在的数据！"}

    store_user_log(
        req, "api.ticket.get_ticket_detail", "get_ticket_detail", "",
        {"ticket_id": body.ticket_id}
    )
...
@wish_response
@wish_checker(["team_is_gaming", "game_start"])
async def send_message(req: Request, body: SendMessageParam, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
    ...
    if secret.PLAYGROUND_MODE:
        store_user_log(req, "api.ticket.send_message", "abnormal", "在 playground 模式下发送神谕消息。",
    ...
    if ticket is None:
        store_user_log(
            req, "api.ticket.send_message", "abnormal", "给不存在的工单发送消息。",
            {"ticket_id": body.ticket_id}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "不存在的数据！"}
    if not user.is_staff and user.team.model.id != ticket.team.model.id:
        store_user_log(
            req, "api.ticket.send_message", "abnormal", "给不属于自己队伍的工单发送消息。",
            {"ticket_id": body.ticket_id}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "不存在的数据！"}
    ...
    if not (1 <= len(body.content) <= 400):
        store_user_log(
            req, "api.ticket.send_message", "abnormal", "消息长度不合法。",
            {"content": body.content[:200]}
        )
        return {"status": "error", "title": "ABNORMAL", "message": "提问内容的长度应当在1到400字之间。"}

    store_user_log(
        req, "api.ticket.send_message", "send_ticket_message", "",
        {"ticket_id": body.ticket_id}
    )

    direction = TicketMessageStore.Direction.TO_PLAYER.name if user.is_staff else TicketMessageStore.Direction.TO_STAFF.name

    rep = await worker.perform_action(glitter.TicketMessageReq(
        client=worker.process_name,
        user_id=user.model.id,
        ticket_id=ticket.model.id,
...
@wish_response
@wish_checker(["team_is_gaming", "game_start"])
async def set_ticket_status(req: Request, body: SetTicketStatusParam, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
    assert worker.game is not None
    assert user.team is not None
    ...
    if secret.PLAYGROUND_MODE:
        store_user_log(req, "api.ticket.set_ticket_status", "abnormal", "在 playground 模式下设置神谕状态。",
    ...
    if not user.is_staff:
        store_user_log(
            req, "api.ticket.set_ticket_status", "abnormal", "玩家试图改变神谕状态。",
            {"ticket_id": body.ticket_id, "ticket_status": body.ticket_status}
        )
    ...
    if body.ticket_status not in ["OPEN", "CLOSED"]:
        store_user_log(
            req, "api.ticket.set_ticket_status", "abnormal", "不存在的状态。",
            {"ticket_id": body.ticket_id, "ticket_status": body.ticket_status}
        )
    ...
    if ticket is None:
        store_user_log(
            req, "api.ticket.set_ticket_status", "abnormal", "给不存在的工单设置状态。",
            {"ticket_id": body.ticket_id, "ticket_status": body.ticket_status}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "不存在的数据！"}
    if not user.is_staff and user.team.model.id != ticket.team.model.id:
        store_user_log(
            req, "api.ticket.set_ticket_status", "abnormal", "给不属于自己队伍的工单设置状态。",
            {"ticket_id": body.ticket_id, "ticket_status": body.ticket_status}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "不存在的数据！"}
    ...
    if not user.is_staff and body.ticket_status == "OPEN":
        store_user_log(
            req, "api.ticket.set_ticket_status", "abnormal", "玩家试图重新打开工单。",
            {"ticket_id": body.ticket_id, "ticket_status": body.ticket_status}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "你不能重新打开工单！异常行为已记录！"}

    store_user_log(
        req, "api.ticket.set_ticket_status", "", "",
        {"ticket_id": body.ticket_id, "ticket_status": body.ticket_status}
    )

    rep = await worker.perform_action(glitter.SetTicketStatusReq(
        client=worker.process_name,
        user_id=user.model.id,
        ticket_id=ticket.model.id,
