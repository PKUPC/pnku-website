...
async def submit_answer(req: Request, body: SubmitParam, worker: Worker, user: Optional[User]) -> Dict[str, Any]:
    ...
    if not (0 < len(body.content) <= 100):
        store_user_log(
            req, "api.puzzle.submit_answer", "abnormal",
            f"答案长度不合法，长度为{len(body.content)}。", {}
        )
    ...
    if puzzle_key is None:
        store_user_log(
            req, "api.puzzle.submit_answer", "abnormal", "unhash puzzle_key 失败。",
    ...
    if team_id != user.team.model.id:
        store_user_log(
            req, "api.puzzle.submit_answer", "abnormal", "提交了其他队伍的 puzzle_key。",
    ...
    if not user.is_staff and puzzle_key not in user.team.game_status.unlock_puzzle_keys:
        store_user_log(
            req, "api.puzzle.submit_answer", "abnormal", "题目未解锁。",
            {"puzzle_key": puzzle_key}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}

    store_user_log(
        req, "api.puzzle.submit_answer", "submit_answer",
        f"", {"puzzle_key": puzzle_key, "content": body.content}
    )

    puzzle = worker.game_nocheck.puzzles.puzzle_by_key[puzzle_key]
    rep = await worker.perform_action(glitter.SubmitAnswerReq(
    ...
    if rep.result is not None:
        ...
        if res_dict["status"] == "success":
            store_user_log(
                req, "api.puzzle.submit_answer", "answer_correct", f"",
            ...
            store_user_log(
                req, "api.puzzle.submit_answer", "milestone", f"",
                {"puzzle_key": puzzle.model.key, "content": body.content}
            )
        return res_dict
...
@wish_response
@wish_checker(["team_is_gaming", "game_start"])
async def get_submissions(req: Request, body: GetSubmissionsParam, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
    assert user.team is not None
    ...
    if puzzle_key is None:
        store_user_log(
            req, "api.puzzle.get_submissions", "abnormal", "unhash puzzle_key 失败。",
            {"body_puzzle_key": body.puzzle_key}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
    # 是其他队伍的 puzzle_key
    if team_id != user.team.model.id:
        store_user_log(
            req, "api.puzzle.get_submissions", "abnormal", "提交了其他队伍的 puzzle_key。",
            {"body_puzzle_key": body.puzzle_key, "real_puzzle_key": puzzle_key, "another_team_id": team_id}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
    # 此时 puzzle_key 一定是存在的
    if not user.is_staff and puzzle_key not in user.team.game_status.unlock_puzzle_keys:
        store_user_log(
            req, "api.puzzle.get_submissions", "abnormal", "题目未解锁。",
            {"puzzle_key": puzzle_key}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}

    store_user_log(
        req, "api.puzzle.get_submissions", "get_submissions",
        f"", {"puzzle_key": puzzle_key}
    )
    ...
    if user.is_staff:
        return {
            ...
            } for idx, sub in enumerate(worker.game_nocheck.submissions_by_puzzle_key.get(puzzle_key, []))][::-1]
        }
    else:
        ...
        return {
            'submissions': [{
                ...
                "info": sub.result.info,
                'timestamp_s': int(sub.store.created_at / 1000),
            } for idx, sub in enumerate(user.team.get_submissions_by_puzzle_key(puzzle_key))][::-1]
        }
...
@wish_response
@wish_checker(["team_is_gaming", "game_start"])
async def get_detail(req: Request, body: GetDetailParam, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
    assert user.team is not None
    ...
    if puzzle_key is None:
        store_user_log(
            req, "api.puzzle.get_puzzle_details", "abnormal", "unhash puzzle_key 失败。",
            {"body_puzzle_key": body.puzzle_key}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
    # 是其他队伍的 puzzle_key
    if team_id != user.team.model.id:
        store_user_log(
            req, "api.puzzle.get_puzzle_details", "abnormal", "提交了其他队伍的 puzzle_key。",
            {"body_puzzle_key": body.puzzle_key, "real_puzzle_key": puzzle_key, "another_team_id": team_id}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
    # 此时 puzzle_key 一定是存在的
    if not user.is_staff and puzzle_key not in user.team.game_status.unlock_puzzle_keys:
        store_user_log(
            req, "api.puzzle.get_puzzle_details", "abnormal", "题目未解锁。",
            {"puzzle_key": puzzle_key}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
    ...
    if not user.is_staff:
        ...
        if user.team.game_status.puzzle_visible_status(puzzle_key) != "unlock":
            store_user_log(
                req, "api.puzzle.get_detail", "abnormal",
                f"试图访问没解锁的题目。",
                {"puzzle_key": puzzle_key}
            )
            return {"status": "error", "title": "NOT_FOUND", "message": "题目不存在"}

    store_user_log(req, "api.puzzle.get_puzzle_details", "get_puzzle_details",
                   "", {"puzzle_key": puzzle_key})
...
@wish_response
@wish_checker(["team_is_gaming", "game_start"])
async def get_hints(req: Request, body: GetHintsParam, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
    assert user.team is not None
    ...
    if puzzle_key is None:
        store_user_log(
            req, "api.puzzle.get_hints", "abnormal", "unhash puzzle_key 失败。",
            {"body_puzzle_key": body.puzzle_key}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
    # 是其他队伍的 puzzle_key
    if team_id != user.team.model.id:
        store_user_log(
            req, "api.puzzle.get_hints", "abnormal", "提交了其他队伍的 puzzle_key。",
            {"body_puzzle_key": body.puzzle_key, "real_puzzle_key": puzzle_key, "another_team_id": team_id}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
    # 此时 puzzle_key 一定是存在的
    if not user.is_staff and puzzle_key not in user.team.game_status.unlock_puzzle_keys:
        store_user_log(
            req, "api.puzzle.get_hints", "abnormal", "题目未解锁。",
            {"puzzle_key": puzzle_key}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
    ...
    store_user_log(
        req, "api.puzzle.get_hints", "open_hint_list",
        f"", {"body_puzzle_key": body.puzzle_key, "real_puzzle_key": puzzle_key}
    )
...
@wish_response
@wish_checker(["team_is_gaming", "game_start"])
async def buy_hint(req: Request, body: BuyHintParam, worker: Worker, user: Optional[User]) \
        -> Dict[str, Any]:
    assert user is not None
    if worker.game_nocheck.is_game_end():
        return {"status": "error", "title": "GAME_END", "message": "活动已结束！"}
    ...
    if user.is_staff:
        store_user_log(
            req, "api.puzzle.buy_hint", "abnormal",
            f"staff 不应该调这个 API。", {"body_puzzle_key": body.puzzle_key}
        )
    ...
    if puzzle_key is None:
        store_user_log(
            req, "api.puzzle.buy_hint", "abnormal", "unhash puzzle_key 失败。",
            {"body_puzzle_key": body.puzzle_key}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
    # 是其他队伍的 puzzle_key
    if team_id != user.team.model.id:
        store_user_log(
            req, "api.puzzle.buy_hint", "abnormal", "提交了其他队伍的 puzzle_key。",
            {"body_puzzle_key": body.puzzle_key, "real_puzzle_key": puzzle_key, "another_team_id": team_id}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
    # 此时 puzzle_key 一定是存在的
    if puzzle_key not in user.team.game_status.unlock_puzzle_keys:
        store_user_log(
            req, "api.puzzle.buy_hint", "abnormal", "题目未解锁。",
            {"puzzle_key": puzzle_key}
        )
        return {"status": "error", "title": "BAD_REQUEST", "message": "谜题不存在！"}
