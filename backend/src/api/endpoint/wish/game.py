...
async def get_area_detail(req: Request, body: GetAreaDetailParam, worker: Worker, user: Optional[User]) \
        -> Dict[str, Any]:
    ...
    if user.is_staff:
        return {
            "status": "success",
            "data": adhoc.get_area_info(body.area_name, user, worker)
        }
    assert user.team is not None
    # 检查是否开始游戏
    if body.area_name != "intro" and not worker.game_nocheck.is_game_begin():
        store_user_log(req, "api.game.get_area_detail", "abnormal", "游戏未开始时调用了 API。",
    ...
    if body.area_name in ["intro", "day1", "day2", "day3"]:
        ...
        if body.area_name not in user.team.game_status.unlock_areas:
            worker.log("debug", "get_area_detail",
...
@wish_response
@wish_checker(["player_in_team"])
async def get_puzzle_list(req: Request, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
    assert user.is_staff or user.team is not None
...
async def game_info(_req: Request, worker: Worker, user: Optional[User]) -> Dict[str, Any]:
    ...
    result_dict = {
        ...
        "team": None if (user is None or user.team is None) else {
        ...
        "game": {
            ...
            # TODO: 大概没用了
            "aboutTemplates": [{key: title} for key, title, effective_after in TEMPLATE_LIST if
                               cur_tick >= effective_after],
...
async def get_schedule(_req: Request, worker: Worker) -> Dict[str, Any]:
    ...
    return {
        'data': [{
            ...
            'name': trigger.name,
            'status': (
                'prs' if trigger.tick == worker.game.cur_tick else
                'ftr' if trigger.tick > worker.game.cur_tick else 'pst'
            ),
...
async def get_board(req: Request, body: GetBoardParam, worker: Worker, user: Optional[User]) -> Dict[str, Any]:
    ...
    if not user.is_staff:
        assert user.team is not None
        if not worker.game_nocheck.is_game_begin():
            store_user_log(req, "api.game.get_board", "abnormal",
                           "游戏未开始时调用了 API。", {"board_key": body.board_key})
            return {"status": "error", "title": "NOT_FOUND", "message": "排行榜不存在！"}
        if board_key not in user.team.game_status.unlock_boards:
            store_user_log(req, "api.game.get_board", "abnormal",
                           "board_key 不在玩家的 unlock_boards 中。", {"board_key": body.board_key})
            return {"status": "error", "title": "NOT_FOUND", "message": "排行榜不存在！"}
        ...
        if not worker.game_nocheck.is_game_begin():
            ...
            for item in rendered["list"]:
                ...
                for puzzle in puzzle_list:
                    ...
                    if (user is not None and user.team is not None and
                            user.team.gaming and
                            user.team.game_status.puzzle_visible_status(puzzle_key) != "lock"):
                        is_lock = False
                    if is_lock:
...
async def game_start(req: Request, body: GameStartParam, worker: Worker, user: Optional[User]) -> Dict[str, Any]:
    ...
    if not worker.game_nocheck.is_intro_unlock() and not user.is_staff:
        store_user_log(req, "api.game.game_start", "abnormal",
                       "序章开放前调用了 API。", {"content": body.content})
    ...
    if reply["status"] == "success" and reply["title"] == "答案正确！":
        ...
        rep = await worker.perform_action(glitter.TeamGameBeginReq(
...
async def get_team_list(_req: Request, worker: Worker, user: Optional[User]) -> Dict[str, Any]:
    ...
    def _recruiting(team: Team) -> bool:
        if (len(team.members) == secret.TEAM_MAX_MEMBER
...
async def get_story_list(req: Request, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
    assert user.team is not None
    ...
    if not user.is_staff:
        if not worker.game_nocheck.is_game_begin():
            store_user_log(req, "api.game.get_story_list", "abnormal",
                           "游戏开始前调用了 API。", {})
            return {"status": "error", "title": "ABNORMAL", "message": "游戏未开始！"}
