...
async def update_profile(req: Request, body: UpdateProfileParam, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    ...
    rep = await worker.perform_action(glitter.UserUpdateProfileReq(
    ...
    store_user_log(req, "api.user.update_profile", "update_profile", "",
                   {"profile": body.profile})  # type: ignore
...
@wish_response
@wish_checker(["user_login"])
async def change_password(req: Request, body: ChangePasswordParam, worker: Worker, user: Optional[User]) \
        -> dict[str, Any]:
    assert user is not None
