...
from .. import secret, store, utils
from .views import VIEWS, DebugView, FilesView, StatusView, TemplateView
...
with app.app_context():
    ...
    for model_name in dir(store):
        if model_name.endswith('Store'):
            ...
            admin.add_view(
                secured(VIEWS.get(model_name, ModelView))(
                    getattr(store, model_name),
                    db.session,
