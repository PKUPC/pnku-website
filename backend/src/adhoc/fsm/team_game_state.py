...
from .special_puzzles import (Day202Status, Day203Status, Day2Meta, Day3Normal, Day320Status,
                              Day3PreMetaStatus, Day206Status, Day201Status)
...
class TeamGameStatus:
            ...
            case "pass":
                ...
                if puzzle_key.startswith("day1"):
                    ...
                    if puzzle_key in ["day3_meta1", "day3_meta2", "day3_meta3"]:
                        ...
                        if (
                                "day3_meta1" in self.passed_puzzle_keys and
                                "day3_meta2" in self.passed_puzzle_keys and
                                "day3_meta3" in self.passed_puzzle_keys
                        ):
                            self.finished = True
    ...
    def test_submission(self, puzzle_key: str, submission: str) -> SubmissionResult:
        ...
        if submission_result.type == "pass":
            ...
            if puzzle_key.startswith("day2") and puzzle_key != "day2_meta" and "day2_meta" in self.passed_puzzle_keys:
                submission_result.pass_after_meta = True
            if (puzzle_key.startswith("day3") and puzzle_key not in ["day3_meta1", "day3_meta2", "day3_meta3"]
                    and self.finished):
                submission_result.pass_after_meta = True
            if self.finished:
    ...
    def add_unlock_puzzle(self, puzzle_key: str, unlock_ts: int, is_reloading: bool) -> None:
        ...
        if not is_reloading:
            self.team.game.worker.log(
                "debug", "team_game_state.add_unlock_puzzle",
                f"team#{self.team.model.id} unlock {puzzle_key}"
            )
        ...
        if not is_reloading:
            if puzzle_key == "day1_meta":
                self.team.game.worker.emit_ws_message({
                    "type": "normal", "to_users": self.team.member_ids,
                    "payload": {
                        "type": "game_message", "title": "元谜题解锁！",
    ...
    def on_puzzle_action(self, event: PuzzleActionEvent) -> None:
        if event.puzzle_key in self.puzzle_status_by_key:
            ...
            self.team.game.log(
                "warning",
                "TeamGameStatus.on_puzzle_action",
                f"Unknown puzzle_key: {event.puzzle_key}"
            )
